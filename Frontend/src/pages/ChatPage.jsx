...
export default function ChatPage() {
  ...
  const handleSend = async () => {
    ...
    if (!activeChat) {
      ...
      if (res.payload?.success) {
        await dispatch(
          fetchChatsForChapter({ roadmapId, moduleId: chapterId }),
        );
      } else {
  ...
  const handleDelete = async (e, chatId) => {
    e.stopPropagation();
    const res = await dispatch(deleteChat({ chatId }));
    if (res.payload?.success) {
  ...
  const handleRename = async (chatId) => {
    ...
    const res = await dispatch(renameChat({ chatId, newTitle: editTitle }));
    if (res.payload?.success) {
          ...
          {isFetching ? (
            ...
            chatData.chats.map((chat) => (
              <div
                ...
                onClick={() =>
                  dispatch(
                    setActiveChat({ moduleId: chapterId, chatId: chat.id }),
                  )
                }
                className={`group flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer transition-colors ${
                  chat.id === chatData.activeChatId
                    ? "bg-slate-800"
