...
export default function RoadmapDisplay() {
    ...
    const { currRoadmap, notes, explanation_loading, quizData, quizLoading } = useSelector(state => state.roadmap);
    ...
    useEffect(() => {
        const load = async () => {
            ...
            if (res.payload?.success) {
                ...
                const expRes = await dispatch(fetchSubtopicExplanation({ roadmapId: id }));
    ...
    const handleProgress = async (chapterId, subtopicId) => {
        const res = await dispatch(saveProgress({ roadmapId: id, chapterId, subtopicId }));
        if (res.payload?.success) dispatch(setcurrRoadmap(res.payload.data));
    };
    ...
    const handleGenerateSummary = async (chapterId, subtopicId) => {
        ...
        const res = await dispatch(generateSubtopicSummary({
    ...
    const getScore = () => {
        ...
        return questions.filter((q, i) => quizAnswers[i] === q.correctAnswer).length;
    };
            ...
            <div className="max-w-4xl mx-auto px-4 pt-28 pb-16">
                ...
                <div className="space-y-4">
                    {roadmap.chapters?.map(chapter => {
                        ...
                        return (
                            <div key={chapter.id} className="bg-slate-800/50 border border-slate-700/50 rounded-2xl overflow-hidden">
                                ...
                                {expandedChapters[chapter.id] && (
                                    <div className="border-t border-slate-700/50 divide-y divide-slate-700/30">
                                        {chapter.subtopics?.map(subtopic => {
                                            ...
                                            const isExpLoading  = explanation_loading.includes(`${chapter.id}:${subtopic.id}`);
                                            const isQuizLoading = quizLoading.includes(`${chapter.id}:${subtopic.id}`);
                                            ...
                                            return (
                                                <div key={subtopic.id} className="px-5 py-4">
                                                    <div className="flex items-start gap-3">
                                                        ...
                                                        {/* Progress Checkbox */}
                                                        <button onClick={() => handleProgress(chapter.id, subtopic.id)}
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            {/* Subtopic Title */}
                                                            <button onClick={() => toggleSubtopic(stKey)}
                                                                className="w-full text-left">
                                                                <div className="flex items-center justify-between gap-2">
                                                                    <h4 className={`font-medium ${subtopic.completed ? 'line-through text-slate-500' : ''}`}>
                                                                        {subtopic.title}
                                                                    </h4>
                                                                    <div className="flex items-center gap-1 shrink-0">
                                                                        <span className="text-xs text-slate-500">{subtopic.estimatedTime}</span>
                                                                        {expandedSubtopics[stKey]
                                                                            ? <ChevronUp className="h-4 w-4 text-slate-400" />
                                                                            : <ChevronDown className="h-4 w-4 text-slate-400" />}
                                                                    </div>
                                                                </div>
                                                                <p className="text-sm text-slate-400 mt-0.5">{subtopic.description}</p>
                                                            </button>
                                                            ...
                                                            {expandedSubtopics[stKey] && (
                                                                <div className="mt-4 space-y-4">
                                                                    ...
                                                                    <div className="flex flex-wrap gap-2">
                                                                        <button onClick={() => openNote(chapter.id, subtopic.id)}
                                                                            ...
                                                                            {notes[noteKey] ? 'Edit Note' : 'Add Note'}
                                                                        </button>
                                                                        <button
                                                                            onClick={() => handleGenerateSummary(chapter.id, subtopic.id)}
                                                                        ...
                                                                        <button
                                                                            onClick={() => handleGenerateQuiz(chapter.id, subtopic.id)}
                                                                        ...
                                                                        <button
                                                                            onClick={() => navigate(`/roadmap/${id}/chat/${chapter.id}`)}
