...
export default function Roadmaps() {
  ...
  const handleDelete = async (e, id) => {
    ...
    const res = await dispatch(deleteUserRoadmap(id));
    if (res.payload?.success) {
      toast.success("Roadmap deleted");
    } else {
      toast.error(res.payload?.message || "Failed to delete roadmap");
    }
  };
  ...
  const handlePin = async (e, id) => {
    ...
    if (res.payload?.success) {
      toast.success(
        res.payload.data.isPinned ? "Roadmap pinned" : "Roadmap unpinned",
      );
    } else {
      toast.error(res.payload?.message || "Failed to update pin");
    }
  };
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-400 mt-1">
              ...
              {userRoadmaps.length !== 1 ? "s" : ""} created
            </p>
          </div>
          <Link
            to="/roadmap/generate"
              ...
              >
                ...
                <div className="flex justify-between items-start mb-4 mt-2">
                  ...
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      onClick={(e) => handlePin(e, roadmap._id)}
                ...
                <div className="flex items-center gap-3 text-xs text-slate-500">
                  <span>
                    {roadmap.roadmapData?.chapters?.length || 0} chapters
                  </span>
                  <span>•</span>
